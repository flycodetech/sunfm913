...
if ( ! function_exists( 'verse_array_get_first' ) ) {
	/**
	 ...
	 */
	function verse_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'verse_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function verse_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'verse_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function verse_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'verse_array_from_list' ) ) {
	/**
	 ...
	 */
	function verse_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'verse_array_slice' ) ) {
	/**
	 ...
	 */
	function verse_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'verse_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * verse_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * verse_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function verse_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'verse_array_insert_after' ) ) {
	/**
	 ...
	 */
	function verse_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = verse_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: verse_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'verse_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function verse_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'verse_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function verse_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'verse_array_str_replace' ) ) {
	/**
	 ...
	 */
	function verse_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = verse_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'verse_generate_id' ) ) {
	/**
	 ...
	 */
	function verse_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'verse_get_tag' ) ) {
	/**
	 ...
	 */
	function verse_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'verse_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function verse_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'verse_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function verse_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( verse_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'verse_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function verse_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'verse_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function verse_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'verse_get_column_class' ) ) {
	/**
	 ...
	 */
	function verse_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'verse_stripslashes' ) ) {
	/**
	 ...
	 */
	function verse_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'verse_get_value_gp' ) ) {
	/**
	 ...
	 */
	function verse_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'verse_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function verse_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'verse_get_value_gps' ) ) {
	/**
	 ...
	 */
	function verse_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'verse_get_session_value' ) ) {
	/**
	 ...
	 */
	function verse_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'verse_set_session_value' ) ) {
	/**
	 ...
	 */
	function verse_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'verse_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function verse_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'verse_get_cookie' ) ) {
	/**
	 ...
	 */
	function verse_get_cookie( $name, $defa = '' ) {
		return verse_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'verse_set_cookie' ) ) {
	/**
	 ...
	 */
	function verse_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'verse_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//verse_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//verse_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'verse' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'verse_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'verse_hex2rgba' ) ) {
	/**
	 ...
	 */
	function verse_hex2rgba( $hex, $alpha ) {
		$rgb = verse_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'verse_hex2hsb' ) ) {
	/**
	 ...
	 */
	function verse_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = verse_rgb2hsb( verse_hex2rgb( $hex ) );
...
if ( ! function_exists( 'verse_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'verse_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'verse_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'verse_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function verse_hsb2hex( $hsb ) {
		return verse_rgb2hex( verse_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'verse_date_to_sql' ) ) {
	/**
	 ...
	 */
	function verse_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'verse_format_price' ) ) {
	/**
	 ...
	 */
	function verse_format_price( $price ) {
...
if ( ! function_exists( 'verse_num2size' ) ) {
	/**
	 ...
	 */
	function verse_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'verse_size2num' ) ) {
	/**
	 ...
	 */
	function verse_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'verse_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'verse_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function verse_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'verse_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function verse_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'verse_is_on' ) ) {
	/**
...
if ( ! function_exists( 'verse_is_off' ) ) {
	/**
...
if ( ! function_exists( 'verse_is_inherit' ) ) {
	/**
	 ...
	 */
	function verse_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'verse_strshort' ) ) {
	/**
	 ...
	 */
	function verse_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = verse_strip_tags( $str );
...
if ( ! function_exists( 'verse_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function verse_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'verse_strip_tags' ) ) {
	/**
...
if ( ! function_exists( 'verse_excerpt' ) ) {
	/**
	 ...
	 */
	function verse_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return verse_strwords( verse_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'verse_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function verse_unserialize_recover( $str ) {
...
if ( ! function_exists( 'verse_unserialize' ) ) {
	/**
	 ...
	 */
	function verse_unserialize( $str ) {
...
if ( ! function_exists( 'verse_str_replace' ) ) {
	/**
	 ...
	 */
	function verse_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = verse_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( verse_str_replace( $from, $to, verse_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'verse_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function verse_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'verse_tags_count' ) ) {
	/**
	 ...
	 */
	function verse_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'verse_getimagesize' ) ) {
	/**
	 ...
	 */
	function verse_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = verse_is_url( $url ) ? verse_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'verse_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function verse_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = verse_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'verse_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function verse_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = verse_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = verse_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = verse_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = verse_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'verse_get_thumb_size' ) ) {
	 ...
	 */
	function verse_get_thumb_size( $ts ) {
		$thumb_sizes = verse_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "verse-thumb-{$ts}" ] ) ) {
			$ts = 'verse-thumb-' . $ts;
		}
		$retina = verse_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'verse_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'verse_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'verse-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'verse_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'verse-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'verse_get_closest_thumb_size' ) ) {
	 ...
	 */
	function verse_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'verse_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function verse_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = verse_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'verse_get_post_image' ) ) {
	/**
	 ...
	 */
	function verse_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'verse_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function verse_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = verse_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = verse_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = verse_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( verse_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = verse_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'verse_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function verse_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'verse_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'verse_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function verse_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = verse_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = verse_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'verse_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'verse_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function verse_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'verse_get_post_video_list_first' ) ) {
	/**
...
if ( ! function_exists( 'verse_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function verse_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = verse_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = verse_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'verse_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'verse_get_embed_video' ) ) {
	/**
	...
	function verse_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = verse_add_to_url( $video, verse_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'verse_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'verse_filter_video_width', 325, 'tiktok' ) : verse_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'verse_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'verse_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function verse_get_embed_video_tag_name() {
...
if ( ! function_exists( 'verse_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function verse_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = verse_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										verse_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'verse_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function verse_is_from_uploads( $url ) {
		$url          = verse_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = verse_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'verse_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function verse_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'verse_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function verse_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'verse_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'verse_init_filesystem', 0 );
	/**
	 ...
	 */
	function verse_init_filesystem( $force = false ) {
...
if ( ! function_exists( 'verse_fpc' ) ) {
	 ...
	 */
	function verse_fpc( $file, $data, $flag = 0 ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'verse' ), $file ) );
					}
				}
			}
		}
	...
	function verse_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? verse_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'verse' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'verse' ), $file ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'verse_mkdir' ) ) {
	/**
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'verse' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'verse' ), $path ) );
					}
				}
			}
		}
...
if ( ! function_exists( 'verse_unlink' ) ) {
	/**
	 ...
	 * @throws Exception        Throw an Exception if the object $wp_filesystem is not initialized yet or if error occurs.
	 */
	function verse_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( verse_is_on( verse_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'verse' ), $path ) );
					}
				}
			}
		}
	...
	function verse_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = verse_prepare_path( $src );
				$dst = verse_prepare_path( $dst );
...
if ( ! function_exists( 'verse_unzip_file' ) ) {
	/**
...
if ( ! function_exists( 'verse_esc' ) ) {
	 ...
	 */
	function verse_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'verse_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'verse_check_min_file' ) ) {
	 ...
	 */
	function verse_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && verse_is_off( verse_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && verse_is_off( verse_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'verse_get_file_dir' ) ) {
	/**
	 ...
	 */
	function verse_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! VERSE_ALLOW_SKINS && ! verse_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'verse_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( VERSE_CHILD_DIR != VERSE_THEME_DIR && file_exists( VERSE_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? VERSE_CHILD_URL : VERSE_CHILD_DIR ) . verse_check_min_file( $file, VERSE_CHILD_DIR );
			} elseif ( file_exists( VERSE_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? VERSE_THEME_URL : VERSE_THEME_DIR ) . verse_check_min_file( $file, VERSE_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'verse_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function verse_get_file_url( $file ) {
		return verse_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'verse_get_file_ext' ) ) {
	/**
	 ...
	 */
	function verse_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'verse_get_file_name' ) ) {
	/**
	 ...
	 */
	function verse_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'verse_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function verse_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'verse_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( VERSE_CHILD_DIR != VERSE_THEME_DIR && is_dir( VERSE_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? VERSE_CHILD_URL : VERSE_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( VERSE_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? VERSE_THEME_URL : VERSE_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'verse_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'verse_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function verse_get_folder_url( $folder ) {
		return verse_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'verse_merge_js' ) ) {
	/**
	 ...
	 */
	function verse_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= verse_fgc( verse_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = verse_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = verse_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			verse_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'verse' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'verse_filter_js_output', apply_filters( 'verse_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'verse_merge_css' ) ) {
	/**
	 ...
	 */
	function verse_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'verse_filter_responsive_sizes', verse_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = verse_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = verse_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'verse_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'verse_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'verse' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'verse' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = verse_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			verse_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'verse') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'verse_filter_css_output', apply_filters( 'verse_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'verse_filter_merge_css' ) ) {
	add_filter( 'verse_filter_merge_css', 'verse_filter_merge_css' );
	/**
	 ...
	 */
	function verse_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'verse_merge_sass' ) ) {
	/**
	 ...
	 */
	function verse_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'verse_filter_responsive_sizes', verse_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'verse_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			verse_fpc( verse_get_file_dir( $to ), apply_filters( 'verse_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'verse_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function verse_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'verse_add_protocol' ) ) {
	/**
	 ...
	 */
	function verse_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? verse_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'verse_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function verse_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'verse_add_to_url' ) ) {
	/**
	 ...
	 */
	function verse_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = verse_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'verse_is_url' ) ) {
	/**
	 ...
	 */
	function verse_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'verse_is_external_url' ) ) {
	/**
	 ...
	 */
	function verse_is_external_url( $url ) {
		return verse_is_url( $url ) && strpos( $url, verse_remove_protocol_from_url( apply_filters( 'trx_addons_filter_home_url', get_home_url() ), true ) ) === false;
	}
}

if ( ! function_exists( 'verse_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function verse_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => verse_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => VERSE_CHILD_DIR,
				'url' => verse_remove_protocol_from_url( VERSE_CHILD_URL )
				),
			'theme' => array(
				'dir' => VERSE_THEME_DIR,
				'url' => verse_remove_protocol_from_url( VERSE_THEME_URL )
				)
			);

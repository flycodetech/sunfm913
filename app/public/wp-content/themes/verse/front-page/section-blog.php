<div class="front_page_section front_page_section_blog<?php
	$verse_scheme = verse_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $verse_scheme ) && ! verse_is_inherit( $verse_scheme ) ) {
		echo ' scheme_' . esc_attr( $verse_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( verse_get_theme_option( 'front_page_blog_paddings' ) );
	if ( verse_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$verse_css      = '';
		$verse_bg_image = verse_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$verse_anchor_icon = verse_get_theme_option( 'front_page_blog_anchor_icon' );
	$verse_anchor_text = verse_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( verse_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$verse_css      = '';
			$verse_bg_mask  = verse_get_theme_option( 'front_page_blog_bg_mask' );
			$verse_bg_color_type = verse_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $verse_bg_color_type ) {
				$verse_bg_color = verse_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$verse_caption = verse_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $verse_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $verse_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $verse_caption, 'verse_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$verse_description = verse_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $verse_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $verse_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $verse_description ), 'verse_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! verse_exists_trx_addons() ) {
						...
						verse_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>

...
if ( ! function_exists( 'verse_customizer_get_css' ) ) {

	function verse_customizer_get_css( $args = array() ) {
		...
		if ( $fonts ) {
			...
			$fonts        = verse_customizer_add_theme_fonts( $fonts );
			$rez          = array();
...
.verse_tabs .verse_tabs_titles li a {
...
CSS;
			$rez          = apply_filters( 'verse_filter_get_css', $rez, array( 'fonts' => $fonts ) );
			$css['fonts'] = $rez['fonts'];
		}
		...
		if ( $vars ) {
			...
			$vars = verse_customizer_add_theme_vars( $vars );

			$rez         = array();
			$rez['vars'] = '';
			$rez         = apply_filters( 'verse_filter_get_css', $rez, array( 'vars' => $vars ) );
			$css['vars'] = $rez['vars'];
		}
		...
		//--------------------------------------
		if ( false !== $colors ) {
			$schemes = empty( $scheme ) ? array_keys( verse_get_sorted_schemes() ) : array( $scheme );
			if ( count( $schemes ) > 0 ) {
				$rez = array();
				foreach ( $schemes as $s ) {
					// Prepare colors
					if ( empty( $scheme ) ) {
						$colors = verse_get_scheme_colors( $s );
					}
					...
					$rez['colors']  = '';
					$rez            = apply_filters(
						'verse_filter_get_css', $rez, array(
							'colors' => $colors,
							'scheme' => $s,
						)
					);
		...
		return apply_filters( 'verse_filter_prepare_css', $css_str, $remove_spaces );
	}
}

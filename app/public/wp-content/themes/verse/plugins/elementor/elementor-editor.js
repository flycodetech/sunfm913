...
jQuery( document ).ready(
	function() {
		...
		verse_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="verse_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.verse_post_editor' );
						if ( link.length > 0 ) {
							verse_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.verse_post_editor', function(e) {
						verse_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('verse_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function verse_change_post_edit_link_elementor(a) {
			if (a.length > 0) {

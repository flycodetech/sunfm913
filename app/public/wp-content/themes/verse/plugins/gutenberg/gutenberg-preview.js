...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			verse_create_observer( 'verse-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				verse_gutenberg_editor_init();
			} );
		} else {
		 	verse_gutenberg_editor_init();
		}
	...
	function verse_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'verse_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'verse_inited' ) ) {
	...
	// Init on page load
	function verse_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = verse_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + VERSE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + VERSE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in VERSE_STORAGE['override_classes'] ) {
				$obj.addClass( VERSE_STORAGE['override_classes'][i].replace( '%s', VERSE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( verse_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && VERSE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'verse_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			verse_remove_observer( 'verse-check-editor-styles-wrapper' );
			verse_create_observer( 'verse-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'verse_inited' );
	}
} );

...
if ( function_exists( 'verse_elementor_is_preview' ) && verse_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			verse_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	verse_new_main_query(
		array(
			'post_type'      => verse_get_theme_option( 'post_type' ),
			'category'       => verse_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'verse_filter_get_template_part', verse_blog_archive_get_template() ) );
}
